...
//
///////////////////////////////////////////////////////////////////////////////
void frustrum_t::init_frustum_planes(float fov_x, float fov_y, float z_near, float z_far) {
...
polygon_t frustrum_t::polygon_from_triangle(vec3_t v0, vec3_t v1, vec3_t v2, tex2_t t0, tex2_t t1, tex2_t t2) {
    polygon_t polygon = {
        { v0, v1, v2 },
        { t0, t1, t2 },
        3
...
void frustrum_t::triangles_from_polygon(polygon_t* polygon, triangle_t triangles[], int* num_triangles) {
...
void frustrum_t::clip_polygon_against_plane(polygon_t* polygon, int plane) {
    ...
    float current_dot = 0;
    float previous_dot = vec3_t::vec3_dot(vec3_t::vec3_sub(*previous_vertex, plane_point), plane_normal);

    // Loop all the polygon vertices while the current is different than the last one
    while (current_vertex != &polygon->vertices[polygon->num_vertices]) {
        current_dot = vec3_t::vec3_dot(vec3_t::vec3_sub(*current_vertex, plane_point), plane_normal);
        ...
        if (current_dot * previous_dot < 0) {
            ...
            inside_vertices[num_inside_vertices] = vec3_t::vec3_clone(&intersection_point);
            inside_texcoords[num_inside_vertices] = tex2_t::tex2_clone(&interpolated_texcoord);
            num_inside_vertices++;
        }
        ...
        if (current_dot > 0) {
            ...
            inside_vertices[num_inside_vertices] = vec3_t::vec3_clone(current_vertex);
            inside_texcoords[num_inside_vertices] = tex2_t::tex2_clone(current_texcoord);
            num_inside_vertices++;
        }
    ...
    for (int i = 0; i < num_inside_vertices; i++) {
        polygon->vertices[i] = vec3_t::vec3_clone(&inside_vertices[i]);
        polygon->texcoords[i] = tex2_t::tex2_clone(&inside_texcoords[i]);
    }
    polygon->num_vertices = num_inside_vertices;
}

void frustrum_t::clip_polygon(polygon_t* polygon) {
